...
SELECT RES_ID from(SELECT * FROM RESERVATION ORDER BY RES_ID DESC) WHERE ROWNUM = 1;
SELECT RES_ID FROM RESERVATION WHERE ROWNUM = 1 ORDER BY RES_ID DESC;
ALTER TABLE RSVTID
	ADD FOREIGN KEY (res_id)
	REFERENCES reservation (res_id) ON DELETE SET NULL
; 
INSERT INTO RSVTID (res_id, rsv_tid) VALUES ((SELECT RES_ID FROM RESERVATION WHERE ROWNUM = 1 ORDER BY RES_ID DESC), 'temp09');
CREATE TABLE RSVTID (res_id number NOT NULL, rsv_tid varchar2(24) NOT NULL, PRIMARY KEY(rsv_tid));
/*좌석*/
CREATE TABLE seats
(
