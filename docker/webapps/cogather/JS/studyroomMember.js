		...
		if(id == msg.sender){
			...
			"<li class='chat chat-right'> "+ 
				"<div class='chat-sender-info'>"+
					"<img class= 'chat-sender-img' src='"+contextPath+"/"+member_profiles[msg.sender]+"'>"+
		...
		success: function(data, status) {
			if (status == "success") {
				...
				for(var i = 0; i<data['mdata'].length;i++){
					member_profiles[data['mdata'][i].id] = data['mdata'][i].pimg_url;
					console.log('id: '+ data['mdata'][i].id +": " + member_profiles[data['mdata'][i].id]);
				}
			}
		}
	});
